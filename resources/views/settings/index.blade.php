...
<div class="container mt-5">
    ...
    @if($selectedUser)
        @php
            // Controller should pass assigned slugs as $assignedSlugs
                    ...
                    @foreach($features as $feature)
                        <tr>
                            ...
                            @foreach($actions as $action)
                                @php $slug = "{$feature}.{$action}"; @endphp
                            ...
                            <td class="text-center">
                                <!-- toggles the row's action checkboxes -->
                                <input type="checkbox" class="toggle-row-all" aria-label="Toggle all for {{ $feature }}">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const permCB = 'input.perm-checkbox';
    ...
    // Row All toggle -> only toggles the permission checkboxes in the same row
    ...
    // Auto update Row-All when individual boxes change
    document.querySelector("table")?.addEventListener("change", e => {
        if (e.target.matches(permCB)) {
            ...
            if (rowAllBox) rowAllBox.checked = boxes.every(cb => cb.checked);
        }
    });
